...
#include "UCSCBEDOperator.h"args[7]
...
int main(int argc, char* args[]) {
	...
	if( argc < 9){
            cout<<"Usage: tags_around_summit53 file_names_file "
            		"input_option input_file upL downL shift out_put input_sense_strand_only[y/n]\n";
		...
		while( !in.eof() ){
			...
			if( !line.empty() ){
				...
				//if( line.find("chr1-")!=std::string::npos)
					smoothed_counts_in_islands(regions,
							line, tag, shift, upL, dnL );
			}
		}
	}else{
		cout<<file_names<<endl;
		smoothed_counts_in_islands(regions, file_names, tag, shift, upL, dnL );
	}
/*	ofstream out5(Str(Str(args[7])+"5").data());
	...
	vector<int> nucs5( upL+dnL, 0), nucs3( upL+dnL, 0);
	...
	for( int i = 0; i < nucs5.size()-step; ++i ){
		...
		out53<<(++NN)<<'\t'<<(i+step/2)<<'\t'<<score/step<<'\t'<<args[7]<<'5'<<endl;
	}
	out53.close();
	...
	for( int i = 0; i < nucs5.size()-step; ++i ){
		...
		out15<<(++NN)<<'\t'<<(i+step/2)<<'\t'<<score/step<<'\t'<<args[7]<<'5'<<endl;
	}
	out15.close();
	...
	for( int i = 0; i < nucs3.size()-step; ++i ){
		...
		out13<<(++NN)<<'\t'<<(i+step/2)<<'\t'<<score/step<<'\t'<<args[7]<<'3'<<endl;
	}
	out13.close();
	out13.close();
